...
if [[ $OPTION == "I" ]]; then
    ...
    sed '4d;5d;8d;104d' ClientAppSettings.json >CAS.json && sed '3s/$/,/' CAS.json >CAS1.json && sed '5s/$/,/' CAS1.json >CAS2.json && sed '100s/$/,/' CAS2.json >ClientAppSettings.json
    rm -rf CAS.json CAS1.json CAS2.json && echo 'Installer finished!' && echo
elif [[ $OPTION == "U" ]]; then
...
if [ ! -f /Applications/Roblox.app/Contents/MacOS/ClientSettings/ClientAppSettings.json ]; then
    ...
    echo "File found, RFO is installed!" && exit 0
fi
